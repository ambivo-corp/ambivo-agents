...
def _create_orchestrator(self) -> ConversationOrchestrator:
 ...
 The ConversationOrchestrator constructor accepts:
 - memory_manager: Optional shared memory manager for conversation history.

 Persistence is configured separately via SQLiteWorkflowPersistence
 (see State Persistence section below).
 """
 ...
 return ConversationOrchestrator(memory_manager=None)
```

**Note:** The `ConversationOrchestrator` constructor only accepts `memory_manager`. For persistence, configure `SQLiteWorkflowPersistence` separately (see the State Persistence section).
...
#### 4. Configuration Loader (Reference)

The following shows how a workflow persistence config loader could be structured. Note that `load_workflow_persistence_config` is not currently part of the codebase -- add it to `config/loader.py` if needed:
...
#### 6. Using SQLite Persistence with the Orchestrator

The `ConversationOrchestrator` constructor accepts only `memory_manager`. To add SQLite persistence, use `SQLiteWorkflowPersistence` alongside the orchestrator:

```python
from ambivo_agents.core.persistence.sqlite_backend import SQLiteWorkflowPersistence
from ambivo_agents.core.workflow_orchestrator import ConversationOrchestrator

# Create orchestrator
orchestrator = ConversationOrchestrator(memory_manager=some_memory_manager)

# Create persistence separately
...
# Use persistence to save/load state around orchestrator calls
