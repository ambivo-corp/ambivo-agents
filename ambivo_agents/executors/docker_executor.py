...
class DockerCodeExecutor:
    ...
    def execute_code(
        ...
        input_files: Dict[str, str] = None, use_shared_dirs: bool = True
    ) -> Dict[str, Any]:
    ...
    def _execute_code_with_shared_dirs(
        ...
        if input_files:
            for filename, source_path in input_files.items():
                resolved_path = self.shared_manager.resolve_input_file(source_path, self.input_subdir)
                if resolved_path:
                    self.shared_manager.copy_to_input(str(resolved_path), self.input_subdir, filename)
