...
async def test_mysql_workflow():
    ...
    try:
        ...
        print("[OK] ModeratorAgent created")
...
async def test_mongodb_workflow():
    ...
    try:
        ...
        await moderator.cleanup_session()
        print("[OK] MongoDB test completed\n")
...
async def test_direct_database_agents():
    ...
    try:
        ...
        print("[OK] Direct tests completed\n")
        
    except Exception as e:
        print(f"[ERROR] Direct tests failed: {e}\n")
...
def check_dependencies():
    ...
    try:
        ...
        print("[OK] mysql-connector-python available")
    except ImportError:
        issues.append("mysql-connector-python")
        print("[ERROR] mysql-connector-python missing")
    
    try:
        import pymongo
        print("[OK] pymongo available")
    except ImportError:
    ...
    try:
        import tabulate
        print("[OK] tabulate available")
    except ImportError:
        issues.append("tabulate")
        print("[ERROR] tabulate missing")
    
    try:
        from ambivo_agents.agents.database_agent import DatabaseAgent
        print("[OK] DatabaseAgent available")
    except ImportError:
        issues.append("ambivo_agents[database]")
        print("[ERROR] DatabaseAgent missing")
    
    if issues:
        print(f"\n[WARN]Missing dependencies: {', '.join(issues)}")
...
async def main():
    ...
    if not check_dependencies():
        print("\n[ERROR] Please install missing dependencies before testing")
        sys.exit(1)
